...
namespace UpsFeedToTable
{
    public class FileOperations
    {
        ...
        public static IEnumerable<string> GetCsvFilesFromDirectory(string directory)
        {
        ...
        {
            var allFiles = GetCsvFilesFromDirectory(filesDirectory);
            var fileNames = allFiles.Select(Path.GetFileName);

            return fileNames.Where(x => !processedFiles.Contains(x));
        }
    }
}